...
with st.sidebar:
    ...
    # Initialize cooldown tracking in session state
    if "last_ingest_time" not in st.session_state:
        st.session_state.last_ingest_time = 0.0

    COOLDOWN_SECONDS = 10.0
    current_time = time.time()
    time_since_last_click = current_time - st.session_state.last_ingest_time
    is_on_cooldown = time_since_last_click < COOLDOWN_SECONDS
    cooldown_left = int(COOLDOWN_SECONDS - time_since_last_click)

    if is_on_cooldown:
        st.button(f"⏳ Cooldown Active ({cooldown_left}s)", disabled=True, help="Please wait before triggering ingestion again to prevent rate limiting.")
    else:
